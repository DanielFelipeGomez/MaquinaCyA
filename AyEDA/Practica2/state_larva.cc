...
/**
 * @brief Constructor de la clase StateLarva, inicializa su identificador char
 * con su inicial en mayuscula
 *
 */
StateLarva::StateLarva() { state_ = 'L'; }
 ...
 * @return int
 */
void StateLarva::Neighbors(const Grid& board, int i, int j) {
...
State* StateLarva::NextState() {
  State* new_state;
  if (num_larva_ > (num_egg_ + num_pupa_ + num_adult_)) {
