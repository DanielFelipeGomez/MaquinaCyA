...
// Se encarga de buscar si una string esta en otra
...
std::string String::DeleteImpar(std::string string) {
  std::string aux;
  for (int i = 0; i < string.length(); ++i) {
    aux += string[i];
    ++i;
...
std::string String::DeletePar(std::string string) {
  std::string aux;
...
// Concatena solo los pares con los impares de otra cadena
std::string String::Modify1(std::string string) {
  std::string result, stringpar, stringimpar;
  stringpar = DeletePar(this->GetString());
  stringimpar = DeleteImpar(string);
  result = stringpar + stringimpar;
  return result;
}


std::string String::Modify2(int n) {
  std::string result, aux, aux2, concatenate;
  aux = DeletePar(this->GetString());
  aux2 = DeleteImpar(this->GetString());
  concatenate = (aux + aux2);
  String new_string(concatenate);
  result = new_string  ^ n;
  return result;
}