...
// Sobrecarga del operador [] para poder trabajar con set
const std::string& Alphabet::operator[](size_t index) const {
  return *std::next(alphabet_.begin(), index);
}

// Sobrecarga del operador de flujo de salida de la clase Alphabet
std::ostream& operator<<(std::ostream& out, const Alphabet& alphabet) { 
  out << "{";
  for (int i{0}; i < alphabet.alphabet_.size(); ++i) {
    if (i == alphabet.alphabet_.size() - 1) {
      out << alphabet[i];
    } else {
      out << alphabet[i] << ", ";
    }
...
// Sobrecarga del operador de flujo de entrada de la clase Alphabet
std::istream& operator>>(std::istream& in, const Alphabet& alphabet) {
  std::string aux;
  in >> aux;
  return in;
}
