...
void ReadFile(std::string file, int opcode, std::string fileout) {
  ...
  while (!file_in.eof()) {
    ...
    Menu(opcode, alpha, str, file_out);
  }
  file_in.close();
  file_out.close();
}
...
void Menu(int opcode, Alphabet alpha, String str, std::ofstream &fileout) {
  switch (opcode) {
    case 1:
      str.LengthWord(fileout);
      break;
    case 2:
      str.ReverseWord(fileout);
      break;
    case 3:
      str.PrefixesWords(fileout);
      break;
    case 4:
      str.SuffixesWords(fileout);
      break;
    case 5:
      str.SubStrings(fileout);
      break;
    // Modificación
    case 6:
      str.LengthWordModify(fileout);
      break;
    default:
